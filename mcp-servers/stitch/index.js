...
/**
 * Local Stitch MCP server — forwards JSON-RPC to https://stitch.googleapis.com/mcp
 *
 * Derived from the open-source `stitch-mcp` package (Apache-2.0), with added support for
 * `STITCH_API_KEY` / `GOOGLE_API_KEY` (X-Goog-Api-Key) in addition to gcloud ADC.
 *
 * Do not commit API keys; set them in Cursor MCP env or your shell only.
 */
...
const log = {
    info: (msg) => console.error(`[stitch-local] ℹ️  ${msg}`),
    success: (msg) => console.error(`[stitch-local] ✅ ${msg}`),
    warn: (msg) => console.error(`[stitch-local] ⚠️  ${msg}`),
    error: (msg) => console.error(`[stitch-local] ❌ ${msg}`),
};
...
async function getProjectId() {
    ...
    throw new Error("Project ID not found. Set STITCH_PROJECT_ID or GOOGLE_CLOUD_PROJECT env var or run: gcloud config set project YOUR_PROJECT");
}
...
async function buildStitchHeaders(projectId) {
    const headers = {
...
async function callStitchAPI(method, params, projectId) {
    ...
    try {
        const response = await fetch(STITCH_URL, {
            method: "POST",
            headers: await buildStitchHeaders(projectId),
...
async function main() {
    try {
        ...
        const apiKey = getStitchApiKey();
        if (apiKey) {
            log.success("Auth: API key (STITCH_API_KEY or GOOGLE_API_KEY)");
        } else {
            try {
                await getAccessToken();
                log.success("Auth verified (gcloud application-default credentials)");
            } catch (e) {
                throw new Error(
                    "Authentication failed. Set STITCH_API_KEY, or run: gcloud auth application-default login"
                );
            }
        }
